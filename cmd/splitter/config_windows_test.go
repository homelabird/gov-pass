...
func TestApplyWindowsJSONConfig_EngineStatsInterval(t *testing.T) {
	cfg := engine.DefaultConfig()
	wc := windowsRunConfig{StatsInterval: defaultStatsInterval}
	splitChunk := 9
	statsInterval := "2m"

	err := applyWindowsJSONConfig(&cfg, &wc, windowsJSONConfig{
		Engine: &engineJSONConfig{
	...
	if cfg.SplitChunk != splitChunk {
		t.Fatalf("SplitChunk = %d, want %d", cfg.SplitChunk, splitChunk)
	}
	if wc.StatsInterval != 2*time.Minute {
		t.Fatalf("StatsInterval = %s, want 2m", wc.StatsInterval)
	}
}

func TestApplyWindowsJSONConfig_TrimsEngineStringFields(t *testing.T) {
	cfg := engine.DefaultConfig()
	wc := windowsRunConfig{StatsInterval: defaultStatsInterval}
	splitMode := " immediate "
	collectTimeout := " 1s "
	flowTimeout := " 2s "
	gcInterval := " 3s "
	shutdownFailOpenTimeout := " 4s "
	adapterFlushTimeout := " 5s "
	statsInterval := " 6s "
	...
	if cfg.SplitMode != engine.SplitModeImmediate {
		t.Fatalf("SplitMode = %s, want immediate", cfg.SplitMode)
	}
	if cfg.CollectTimeout != time.Second {
		t.Fatalf("CollectTimeout = %s, want 1s", cfg.CollectTimeout)
	}
	if cfg.FlowIdleTimeout != 2*time.Second {
		t.Fatalf("FlowIdleTimeout = %s, want 2s", cfg.FlowIdleTimeout)
	}
	if cfg.GCInterval != 3*time.Second {
		t.Fatalf("GCInterval = %s, want 3s", cfg.GCInterval)
	}
	if cfg.ShutdownFailOpenTimeout != 4*time.Second {
		t.Fatalf("ShutdownFailOpenTimeout = %s, want 4s", cfg.ShutdownFailOpenTimeout)
	}
	if cfg.AdapterFlushTimeout != 5*time.Second {
		t.Fatalf("AdapterFlushTimeout = %s, want 5s", cfg.AdapterFlushTimeout)
	}
	if wc.StatsInterval != 6*time.Second {
		t.Fatalf("StatsInterval = %s, want 6s", wc.StatsInterval)
	}
}

func TestApplyWindowsJSONConfig_TrimsWinDivertFilter(t *testing.T) {
	cfg := engine.DefaultConfig()
	wc := windowsRunConfig{Filter: defaultWinDivertFilter}
	filter := " outbound and ipv6 and tcp.DstPort == 443 "

	err := applyWindowsJSONConfig(&cfg, &wc, windowsJSONConfig{
		WinDivert: &winDivertJSONConfig{
			Filter: &filter,
		},
	})
	if err != nil {
	...
	if wc.Filter != strings.TrimSpace(filter) {
		t.Fatalf("Filter = %q, want trimmed %q", wc.Filter, strings.TrimSpace(filter))
	}
}
